...
const OffersCard = ({ id, logo, name, address, email, phone, discounts }) => {
  return (
    <Link to={`/partners/${id}`} >
    <Card className="nearby-offers-card ">
      ...
      <Card.Body className="pt-0">
        ...
        <Card.Text className="line-clamp-1"><i class="bi bi-geo-alt"></i>&nbsp;&nbsp;{address}</Card.Text>
        ...
        <Card.Text><a className='fs-6 fw-light text-dynamic-white'  href={`href=${phone}`}><i class="bi bi-telephone"></i>&nbsp;&nbsp;{phone}</a></Card.Text>
        {discounts && discounts.map((dics)=>(
        <Card.Text className="text-warning">{dics.discount}% discounts</Card.Text>
        ))}
      </Card.Body>
    </Card>
    </Link>
