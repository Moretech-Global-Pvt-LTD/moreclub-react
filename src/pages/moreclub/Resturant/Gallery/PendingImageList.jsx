...
const PendingImagelist = () => {
  ...
  return (
    <div className="d-flex  flex-wrap gap-2">
      {gallery.userGalleryPending &&
        gallery.userGalleryPending.map((item, index) => (
          <ImageAccept key={item.id} item={item} />
        ))}
      {gallery.userGalleryPending.length === 0 && (
        <div className="text-dynamic-white">No Pending Images</div>
      )}
    </div>
  );
};
